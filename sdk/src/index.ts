...
export class SolanaStablecoin {
  ...
  static async create(connection: Connection, params: CreateStablecoinParams): Promise<SolanaStablecoin> {
  ...
  static async fetch(connection: Connection, mint: PublicKey): Promise<SolanaStablecoin | null> {
    const [config] = await PublicKey.findProgramAddress(
    ...
    const authority = new PublicKey(configInfo.data.slice(8, 40));
    const preset = configInfo.data[40];

    return new SolanaStablecoin(connection, mint, config, authority, preset as Preset);
  }
  ...
  async getTotalSupply(): Promise<number> {
    ...
    const data = mintInfo.value.data as { parsed: { info: { supply: { uiAmountString: string } } } };
...
export class ComplianceClient {
  ...
  async blacklistAdd(address: PublicKey, reason: string): Promise<string> {
    ...
    tx.recentBlockhash = (await new Connection('').getLatestBlockhash()).blockhash;
