...
mod tests {
    use crate::services::{
        ...
        events::{EventIndexer, EventType, OnChainEvent},
        mint_burn::{MintBurnConfig, MintBurnService},
        rpc::RpcClient,
        solana::PROGRAM_ID,
    };
    ...
    async fn test_compliance_service() {
        ...
        let add_result = service
            .add_to_blacklist("BlacklistedAddr".to_string(), "Test blacklist".to_string(), "admin".to_string())
            .await;
    ...
    fn test_solana_service_initialize_and_mint() {
        ...
        let private_key = std::env::var("PRIVATE_KEY_BASE58").expect("PRIVATE_KEY_BASE58 not set");
        ...
        let (mint, config) = service.initialize(Some(1_000_000_000_000), 6).expect("Initialize failed");
